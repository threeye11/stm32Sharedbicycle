...
unsigned char Count_timer_2 = 0;
unsigned char Flag_timer_Accumulated_Mileage_1S = 0;
bool Flag_state_Accumulated_Mileage = 0;
unsigned char rtc_hour = 0;
unsigned char rtc_min = 0;
unsigned char rtc_sec = 0;
static unsigned char rtc_tick = 0;
...
{		
	if(TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
	{
		...
		if(Flag_state_Accumulated_Mileage == 1)
		{
			if(Count_timer_2 < 10)
			{			
				Count_timer_2++;
			...
			{
				Flag_timer_Accumulated_Mileage_1S = 1;
				Count_timer_2 = 0;
			}
		}
